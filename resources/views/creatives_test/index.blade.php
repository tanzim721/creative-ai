...
@section('content')
    <x-app-layout>
        ...
        @if (auth()->user()->role == 1 || auth()->user()->role == 2)
            <div class="dashboard">
                <div class="text-sm header bg-[#2D3A43]">Create creative with AI</div>
                <div class="dashboard_main contain overflow-x-hidden">
                                        ...
                                        <div class="mt-2 bg-transparent gradient_border chat_box">
                                            ...
                                            <div class="flex gap-2 mt-2">
                                                ...
                                                <select id="aiActionSelect"
                                                    class="border px-3 py-1 rounded-md bg-transparent cursor-pointer">
                                                    <option disabled selected>Generate with AI</option>
                                                    <option value="userChoseGenerateImage"
                                                        class="text-black cursor-pointer">Generate Image with AI</option>
                                                    {{-- <option value="userChoseGenerateVideo" class="text-black cursor-pointer">Generate Video with AI</option> --}}
                                                </select>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="d-flex flex-wrap align-items-center justify-content-end" id="generatedImage">
                                @foreach ($imageUrls as $image)
                                    <div class="mt-2 bg-transparent gradient_border chat_box flex flex-col items-center animate__animated animate__fadeInRight py-4"
                                        style="animation-delay: 0.{{ $loop->index + 1 }}s; max-width: 270px; height: auto; margin: 10px; clip-path: inset(0 0 6% 0);"
                                        ...
                                        <img src="{{ $image['url'] }}" alt="{{ $image['style'] }} Image"
                                            ...
                                            style="width: 250px; height: auto; clip-path: inset(0 0 6% 0);" />

                                        <div class="flex justify-between mt-3 w-full">
                                            <!-- Download Button -->
                                            <a href="{{ $image['url'] }}"
                                                onclick="event.preventDefault(); downloadClippedImage('{{ $image['url'] }}', 'generated_{{ strtolower(str_replace(' ', '_', $image['style'])) }}')"
                    ...
                    <!-- user choose generate video with ai -- monjur -->
                    <div id="promtBoxContainer3" class="d-none">
                        <div class="d-flex align-items-center text-right justify-content-end">
                            <div
                                class="mt-2 bg-transparent gradient_border chat_box flex flex-col items-start animate__animated animate__fadeInRight">
                                <p>Generate Video with AI</p>
                            </div>
                        </div>

                        <!-- Video Generation Form -->
                        <div id="videoGenerationForm">
                            <div class="d-flex align-items-center text-right justify-content-start" id="promtBox3">
                                <div class="bg-transparent gradient_border chat_box flex flex-col items-start animate__animated animate__fadeInLeft"
                                    style="animation-delay: 0.15s;">
                                    <p>Give your video prompt below:</p>
                                    <div class="flex flex-col">
                                        <textarea class="border-2 min-h-24 border-blue-700 rounded-lg px-2 py-1 mt-2 bg-blue-200 text-black w-80"
                                            id="question3" name="question3" placeholder="Describe the video you want to generate..."
                                            aria-label="Video Prompt Input" oninput="updateCharCount3()"></textarea>
                                        <p id="charCount3" class="text-sm text-gray-700 mt-1">
                                            @if (old('question3'))
                                                {{ strlen(old('question3')) }}@else{{ 0 }}
                                            @endif / 30 characters required
                                        </p>
                                        <button type="button" class="border px-2 py-1 rounded-md hover:bg-blue-500 mt-2"
                                            id="submitPrompt3">Generate Video</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Loading Animation -->
                        <div class="d-flex align-items-center text-right justify-content-end d-none" id="imageLoading3">
                            <div class="w-56 h-44 mt-2 bg-transparent gradient_border chat_box flex flex-col items-center justify-center animate__animated animate__fadeInRight py-4"
                                style="animation-delay: 0.15s;">
                                <div class="loader"></div>
                                <h1 class="mt-3" id="loadingMSG3">Generating Video...</h1>
                            </div>
                        </div>

                        <!-- Generated Video Display -->
                        <div class="flex align-items-center text-right justify-content-end d-none" id="genaratedVideo">
                            <div
                                class="bg-transparent gradient_border px-4 py-4 inline-block animate__animated animate__fadeInRight">
                                <video width="300" height="300" controls class="object-cover rounded-lg">
                                ...
                                <div class="flex justify-between mt-3 w-full">
                                    <button
                                        class="download-btn border px-3 py-2 text-white rounded-md hover:bg-blue-500 hover:text-white w-1/2 text-center block transition-all duration-300 ease-in-out"
                                        onclick="downloadVideo()">
                                        Download
                                    </button>
                                    <button
                                        class="border text-white px-3 py-2 rounded-md hover:bg-red-500 hover:text-white w-1/2 text-center block transition-all duration-300 ease-in-out ml-2"
                                        onclick="rejectVideo()">
                                        Reject
                                    </button>
                                </div>
                        ...
                        <!-- Error Display -->
                        <div class="w-full text-right text-red-500 py-2 d-none" id="videoError">
                            <h1 id="videoErrorMsg">Failed to generate video. Please try again.</h1>
                        </div>
                    </div>
                </div>
            </div>
        @else
            <div class="flex flex-col items-center justify-center h-screen">
                <div class="bg-[#1a2633] p-8 rounded-lg shadow-lg flex flex-col items-center" style="border-radius: 10px;">
                    <svg class="w-16 h-16 text-yellow-400 mb-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <p class="mt-2 text-2xl text-white font-semibold text-center">
                       To get access, please click "Pay Now" and select your plan or await for admin approval.
                    </p>
                    <a href="127.0.0.1:8000/#pricing"
                       class="mt-6 inline-block bg-gradient-to-r from-blue-500 to-green-400 text-white font-bold py-3 px-8 rounded-full shadow-lg hover:scale-105 transition-transform duration-200">
                        Pay Now
                    </a>
                    <p class="mt-4 text-white text-center">
                        Need assistance? Contact us via email:
                        <a href="mailto:it@toucan-ai.com" class="text-info underline">help@toucan-ai.com</a>
                    </p>
                </div>
            </div>
        @endif
